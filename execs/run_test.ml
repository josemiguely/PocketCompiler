...
let test_parse_lambda () =
  ...
  (Lambda (["x" ; "y"], Prim2 (Add, Id ("x",-1), Id ("y",-1),-1),-1))
...
let test_parse_lambda_apply () =
  ...
  (LamApply ((Lambda (["x" ; "y"], Prim2 (Add, Id ("x",-1), Id ("y",-1),-1),-1)), [Num (7L,-1) ; Num (13L,-1)],-1))
...
let test_parse_letrec () =
  ...
  (LetRec (["f", ["x" ; "y"], Prim2 (Add, Id ("x",-1), Id ("y",-1),-1) ; "g", [], Num (21L,-1)], LamApply (Id ("f",-1), [LamApply (Id ("g",-1), [],-1) ; LamApply (Id ("g",-1), [],-1)],-1),-1))
...
let test_interp_lambda_empty () =
  ...
  (string_of_val (interp (Lambda ([], Num (1L,-1),-1)) empty_env empty_fenv))
  (string_of_val (ClosureV (0, (fun _ -> NumV 5L))))

let test_interp_lambda () =
  check string "lambda is parsed"
  (string_of_val (interp (Lambda (["x" ; "y"], Prim2 (Add, Id ("x",-1), Id ("y",-1),-1),-1)) empty_env empty_fenv))
  (string_of_val (ClosureV (2, (fun _ -> NumV 5L))))
  
let test_interp_lambda_apply () =
  check value "lambda application is parsed"
  (interp (LamApply ((Lambda (["x" ; "y"], Prim2 (Add, Id ("x",-1), Id ("y",-1),-1),-1)), [Num (7L,-1) ; Num (13L,-1)],-1)) empty_env empty_fenv)
  (NumV 20L) 
  
let test_interp_letrec_empty () =
  check value "empty letrec is parsed"
  (interp (LetRec ([], Num (7L,-1),-1)) empty_env empty_fenv)
  (NumV 7L) 

let test_interp_letrec () =
  check value "letrec is parsed"
  (interp (LetRec (["f", ["x" ; "y"], Prim2 (Add, Id ("x",-1), Id ("y",-1),-1) ; "g", [], Num (21L,-1)], LamApply (Id ("f",-1), [LamApply (Id ("g",-1), [],-1) ; LamApply (Id ("g",-1), [],-1)],-1),-1)) empty_env empty_fenv)
  (NumV 42L)
