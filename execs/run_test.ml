...
let test_interp_num () =
  check value "same int" (interp (Num (42L,-1)) empty_env) (NumV 42L)

let test_interp_var () =
  check value "same int" (interp (Id ("x",-1)) ["x", NumV 7L]) (NumV 7L)

let test_interp_compound () =
  check value "same int"
    (interp (Prim2 (Add, (Prim2 (Add, Num (3L,-1), (Prim1 (Sub1, Num (6L,-1),-1)),-1)), Num (12L,-1),-1)) empty_env)
    (NumV 20L)
