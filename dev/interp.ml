...
let rec interp expr env fenv =
  match expr with
  | Id (x,_) -> lookup_env x env
  | Num (n,_) -> NumV n
  | Bool (b,_) -> BoolV b
  | Prim1 (op, e,_) -> 
    (match op with
    | Not -> (let res = interp e env fenv in
      match res with
    ...
    | Print -> (interp (Apply ("print", [e],-1)) env fenv)) (* re-route interp of print to the fake sys interface *)
  | Prim2 (op, e1, e2,_) -> 
    (match op with
    ...
    | Mult -> liftIII (Int64.mul) (interp e1 env fenv) (interp e2 env fenv)
